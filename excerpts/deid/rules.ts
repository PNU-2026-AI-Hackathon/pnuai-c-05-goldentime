...
const recognizers: Recognizer[] = [
  ...
  // 실측(2026-07-19 실차트): 병원 발급본은 뒷자리를 이미 가려 "000000 - *******"로 나온다.
  // 앞 6자리 = 생년월일 그 자체라 남기면 재식별 재료 — 마스킹된 뒷자리와 함께 오면 잡는다.
  { kind: 'rrn', pattern: /\b(\d{6})\s?-\s?\*{4,}/gd, group: 1 },
  { kind: 'rrn', pattern: /(?:주민(?:등록)?번호|생년월일)[:\s]*(\d{6})\b/gid, group: 1 },
  ...
  // 실측: 병원 발급본 등록번호는 하이픈 구획(000-00-00-0) — 위 연속숫자 패턴이 놓친다.
  { kind: 'mrn', pattern: /(?:MRN|등록번호|환자번호|차트번호|병록번호|진료번호)[:\s]*(\d{2,4}(?:-\d{1,4}){2,4})/gid, group: 1 },
  { kind: 'mrn', pattern: /\b\d{3}-\d{2}-\d{2}-\d\b/gd }, // 라벨 없이 반복되는 같은 형식(표 셀)
  { kind: 'mrn', pattern: /(?:발행번호|발급번호|접수번호)[:\s]*([\w-]{4,})/gid, group: 1 },
  ...
  // 주소(address): 라벨 뒤 값(줄 끝까지).
  // ⚠️ "주소"는 의무기록에서 主訴(chief complaint)로도 쓰인다 — 실측(2026-07-19): "주소: 복통 3일째"가
  // 통째로 마스킹돼 임상 정보가 사라졌다. 그래서 애매한 "주소"는 값이 행정구역 패턴을 포함할 때만
  // 주소로 인정하고, 뜻이 하나뿐인 라벨(주소지·현주소·거주지)은 값 검증 없이 인정한다.
  { kind: 'address', pattern: /(?:주소지|현주소|거주지)[:\s]*([^\n]{2,80}?)(?=\s{2}|\n|$)/gd, group: 1 },
  { kind: 'address', pattern: /주\s*소[:\s]*((?=[^\n]*[가-힣]{2,}(?:시|도|군|구|읍|면|동|리|로|길|가)\s*\d)[^\n]{2,80}?)(?=\s{2}|\n|$)/gd, group: 1 },
