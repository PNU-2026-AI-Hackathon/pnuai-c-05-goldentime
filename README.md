...
> 📄 **활용 도구·범위·검증 방식 상세: [`docs/ai-usage.md`](docs/ai-usage.md)**
...
### 4.4. 모델 선택 — 개발사를 쪼갠 3계열

**설계 요점은 성능이 아니라 독립성입니다.** 교차검증의 가치는 세 모델의 오류가
**서로 상관되지 않는다**는 데서 나옵니다. 같은 개발사·같은 아키텍처 계열의 모델은
같은 지점에서 함께 틀리기 때문에, 셋이 합의해도 검증이 되지 않습니다.

그래서 원내 3자리를 **개발사 기준으로** 나눴습니다.

| 자리 | 모델 | 개발사 | 라이선스 |
|---|---|---|---|
| 범용 · 처리량 | `Qwen3.8-27B` | Alibaba | **Apache 2.0** |
| 의료 특화 | `MedGemma 27B` | Google | Health AI Developer Foundations 약관 |
| 제3계열 | Mistral 계열 (`Ministral 3 14B` 등) | Mistral AI | **Apache 2.0** |

> 같은 이유로 **Gemma 4는 제외했습니다.** 성능은 충분하지만 MedGemma와 계열이 겹쳐
> 독립성이 깨집니다. 한국어 강점을 기대한 **EXAONE도 제외**했습니다 — 온프렘 체급인
> 4.0/4.5(32B·33B)는 `EXAONE AI Model License 1.2-NC`로 **비상업**이고, 상용 가능한
> `K-EXAONE-2.0`은 750B MoE라 병원 단일 서버에 올라가지 않습니다. 한국어 적응은
> 별도 모델을 늘리는 대신 **QLoRA(4.3)로 해결**하는 쪽을 택했습니다.
>
> ⚠️ **MedGemma는 Apache 2.0이 아닙니다.** Health AI Developer Foundations 약관은
> 직접적인 환자 진단·치료 용도에 **의료기기 승인**을 요구하고 이 제약이 제3자에게
> 승계됩니다. 상용 배포 전 별도 법적 검토가 필요한 항목으로 인지하고 있습니다.

이 중 앞의 두 자리는 체급이 같아 **동일한 A6000 위에서 교대 적재하며 직접 비교**할 수 있습니다.
상세 비교는 아래와 같습니다.
...
**현재 상태 — 연구계획서를 작성하며 IRB 신청을 준비하는 단계입니다 (2026년 8월 기준).
아직 제출하지 않았고, 따라서 심의도 승인도 시작되지 않았습니다. 승인 전까지는
합성 데이터만 사용합니다.**

연구책임자는 병원 소속 교수이며, 학생 팀은 공동연구원으로 참여합니다.

아래 표는 **승인을 전제로 설계한 계획**이며, 실행된 내용이 아닙니다.

계획서에 담은 설계:

| 항목 | 내용 |
|---|---|
| 대상 | **가정의학과 대표 진단군 1종**, 만 19세 이상 **최대 100명**의 후향적 비정형 EMR (2020.01.01 ~ 2024.12.31) |
| 범위 | 차트 검토 **보조**를 위한 정보 추출·구조화·요약으로 한정. 진단·처방·치료결정 자동화와 환자 직접 제공 권고는 **포함하지 않음** |
| 동의 | 서면동의 면제 신청 *(생명윤리법 제16조 제3항 제1호·제2호)* — 가명자료만 제공받아 대상자를 특정·접촉할 수단이 없고, 접촉 가능한 대상만 선별하면 표본 편향이 발생 |
| 가명처리 | **의료빅데이터센터가 수행**하며, 연구진은 **재식별용 키 파일에 접근하지 않음** |
| 처리 환경 | 승인된 **원내 폐쇄형 분석환경**에서만 처리. 외부 네트워크 송신·외부 매체 반출 금지 |
| 파생물 보호 | 임베딩·검색 인덱스·**학습된 LoRA 어댑터**도 보호대상으로 취급해 반출하지 않음 |
| 정답셋 | **2계층** — 정밀 정답셋(최종평가 전용) / 확장 라벨셋(학습·개발 전용). 정밀 정답셋 대상 환자는 학습·프롬프트 조정·모델선정에서 **전면 제외**(환자 단위 분할) |
| 편향 통제 | 임상자문 교수가 **AI 출력을 열람하지 않은 상태에서** 원문만으로 정답 작성 |
| 평가 단위 | 판정 단위는 환자가 아니라 **추출 항목 9종**. 총 판정 건수는 자료 분양 후 확인되므로 계획 단계에서 확정하지 않고 결과와 함께 보고 |
| 오류 유형 | 근거 없는 생성 / 누락 / 잘못된 상태·시점 연결 / 과도한 단정 — 각각 **별도 유형으로 기록** |
| 보관 | 연구 종료일로부터 **3년**, 이후 기관의 승인된 영구 삭제 절차로 폐기 |

> 이 설계는 **가정의학과 단일 진단군 파일럿**입니다. 초기 구상은 5개 분과였으나,
> 임상자문 교수 1인이 정답셋을 직접 검수해야 하는 구조라 **검수 가능 범위에 맞춰
> 단일 진단군으로 좁혔습니다.** 정량 결과는 확정적 성능 주장이 아니라 후속 확대
> 연구를 위한 **예비 추정치**로 해석합니다.
...
| 02:16 | **그동안 만든 것** — 코퍼스 구축 · 실행 기록 · 사고 리포트 공개 |
| 02:23 | **앞으로 ①** — 온프렘 3-모델 교차검증 (단일 A6000 48GB) |
| 02:31 | **앞으로 ②** — 편향 측정 공개 · QLoRA 분과 적응 |
| 02:38 | **앞으로 ③** — IRB · 데이터 거버넌스 |
| 02:46 | 마무리 |

02:16 이후 네 장면은 전부 **`계획` 뱃지**를 달아 이미 구현된 기능과 시각적으로 분리했습니다.
아직 실현하지 않은 항목을 실적처럼 보이게 하지 않기 위한 장치입니다.
...
임상에서 당연하게 여기던 기준들이 막상 설명하려니 잘 정리되지 않는다는 걸 이번에 알았습니다.
무엇이 먼저 봐야 할 정보인지 매번 다시 따져보게 됐고, 그 과정에서 제가 안다고 생각했던 것들을
다시 확인하게 됐습니다. 팀을 이끄는 자리였지만 실제로는 배운 쪽에 가까웠습니다.
전공이 다른 사람들과 하나의 결과물을 만들어 본 것은 처음이라 어려웠고, 그만큼 남는 것도 많았습니다.

**김용하** · 데이터사이언스전공

처음에는 기능을 하나라도 더 붙이는 게 좋은 줄 알았는데, 진행할수록 덜어내는 판단이 더
어렵다는 걸 느꼈습니다. 결과가 맞는지 확인하는 절차를 만드는 데 예상보다 훨씬 많은 시간이
들었습니다. 혼자 할 때는 그냥 넘어갔을 부분을 팀에서 계속 되묻는 과정이 번거로우면서도
결국 도움이 됐습니다. 잘 만드는 것보다 왜 이렇게 만들었는지 설명할 수 있는 쪽이 더 중요하다는
생각을 하게 됐습니다.

**박보은** · 디자인앤테크놀로지전공

화면을 보기 좋게 만드는 일과 이해되게 만드는 일이 서로 다른 문제라는 걸 계속 확인했습니다.
의료 쪽 내용은 제가 잘 모르는 영역이어서 팀원들에게 자주 물어봐야 했고, 설명을 들으면서
화면 구조를 여러 번 다시 짰습니다. 일정 관리도 같이 맡았는데 계획대로 되는 일이 많지 않아
대부분 조정하는 데 시간을 썼습니다. 전공이 다른 사람들이 모인 팀에서는 중간에서 말을 옮기는
역할이 생각보다 중요하다는 걸 알게 됐습니다.
